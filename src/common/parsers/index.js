// Shared DOM parsers. Pure functions — no side effects — so they can be
// called from:
//   - the Chrome offscreen document (service-worker can't DOMParser)
//   - the Firefox background event page (has DOMParser directly)
//
// Keep in sync with the request/response types the offscreen listener
// registers in src/offscreen/index.js.

import { parseFictionDetails as _parseFictionDetails } from '../utils/fictionDetails.js';

export const parseFictionDetails = _parseFictionDetails;

// Parse fiction page to get title and chapter list.
export function parseChapterList(html, fictionId) {
...
export function parseChapterNotes(html, chapterUrl) {
  const parser = new DOMParser();
  const doc = parser.parseFromString(html, 'text/html');
...
export function extractShoutouts(html, excludeFictionId) {
  const parser = new DOMParser();
